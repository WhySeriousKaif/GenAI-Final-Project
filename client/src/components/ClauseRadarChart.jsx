...
const CustomTooltip = ({ active, payload, isDark }) => {
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${isDark ? '#1d1f24' : '#dbe1e8'}`,
      borderRadius: '10px',
      padding: '10px 14px',
      fontSize: '11px',
      ...
      {d.section && (
        <div style={{ color: isDark ? '#80838d' : '#64748b', marginTop: 4 }}>
          {d.section}
        </div>
      )}
    </div>
...
const ClauseRadarChart = ({ clauses = [], onSelect }) => {
  ...
  // Theme colours
  const radarFill     = isDark ? 'rgba(59,130,246,0.18)'  : 'rgba(37, 99, 235, 0.14)';
  const radarStroke   = isDark ? '#3b82f6'                : '#2563eb';
  const gridStroke    = isDark ? '#1d1f24'                : '#dbe1e8';
  const radiusColor   = isDark ? '#4c4f59'                : '#94a3b8';

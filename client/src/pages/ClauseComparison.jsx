...
const ClauseComparison = () => {
  ...
  const fetchContractsList = async () => {
    try {
      ...
      if (data.success) setContractsList(data.contracts);
  ...
  const handleCompareSubmit = async () => {
    ...
    try {
      ...
      const results = await Promise.all(selectedContractIds.map(id => getContractById(id)));
      setComparedContracts(results.filter(res => res.success).map(res => res.contract));
    } catch (err) { console.error(err); alert('Failed to retrieve detail comparison records.'); }
  ...
  return (
    <div className="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 bg-canvas text-body font-sans">
          ...
          {loadingList ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-48 overflow-y-auto pr-2">
              {contractsList.map(contract => {
                ...
                return (
                  <button key={contract._id} onClick={() => handleToggleContract(contract._id)}
                    className={`flex items-center gap-3 p-3 rounded-xl border text-left transition-all ${isChecked ? 'bg-primary/10 border-primary text-ink' : 'bg-canvas border-hairline text-body hover:border-primary/40'}`}>
                    <div className={`w-4 h-4 rounded flex items-center justify-center border transition-all ${isChecked ? 'bg-primary border-primary text-white' : 'border-muted'}`}>
      ...
      <div className="flex justify-center">
        <button onClick={handleCompareSubmit} disabled={selectedContractIds.length === 0 || selectedClauses.length === 0 || loadingComparison}
                ...
                {selectedClauses.map(clauseType => (
                  <tr key={clauseType} className="hover:bg-surface-cream-strong/30 transition-colors">
                    ...
                    {comparedContracts.map(contract => {
                      ...
                      if (!clause) return (
                        <td key={contract._id} className="py-5 px-4 text-muted italic align-top">Clause not detected in this contract.</td>
                      );
                      return (
                        <td key={contract._id} className="py-5 px-4 space-y-2.5 align-top border-l border-hairline">
                          ...
                          <div className="relative group">
                            ...
                            <p className="text-body line-clamp-4 leading-relaxed hover:line-clamp-none transition-all duration-200 cursor-pointer" title="Click to expand">
                              "{clause.clauseText}"
                            </p>
                          </div>
